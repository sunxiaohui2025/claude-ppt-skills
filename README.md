...
这个 Skill 不是让智能体从零手写一个 HTML PPT。

正确流程只有一个：

1. 生成或修改 `sources/` 下的模块化源文件。
2. 运行 `scripts/merge_deck.py` 合并。
3. 运行 `scripts/validate_deck_contract.py` 校验。
4. 只交付合并后的 `index.html`。

以下都属于错误输出：

- 自己写一个完整 `<!doctype html>`。
- 用 `.slide { min-height: 100vh; }` 做纵向滚动页面。
- 用 `scrollIntoView` 自己实现翻页。
- 没有 `overviewGrid`。
- 没有 `editToolbar`。
- 没有底部 `pageText`、`prevBtn`、`nextBtn`。
- 每页没有 `data-source` 和 `data-slide-id`。
- 每页没有 `.slide-stage`。
- 自己写 `<script>` 替换标准 runtime。

哪怕页面看起来像 PPT，只要没有走标准模板和合并脚本，就不是合格产物。

## 核心能力

- 生成高端沙丘色宣讲风格 HTML PPT。
- 生成前先收集风格、受众、场景、目标和页数等关键信息。
- 根据用户需求先生成大纲，并在用户确认后再开始生成 PPT。
...
切换风格时只需要修改 `stylePack`，例如：

```json
{
  "title": "AI 产品发布",
  "stylePack": "indigo-porcelain"
}
```
...
- 生成前必须先进行需求确认，不能拿到一个标题就直接生成 PPT。
- 必须先确认风格，默认使用 `dune` 沙丘色，未来可以扩展更多风格。
- 必须先输出目标受众、主旨、章节结构、页级大纲和结尾思想，让用户确认。
- 如果用户需求很简单，要先补全合理大纲和叙事线，并标注假设。
- 如果用户提供了详细大纲，要尊重用户原意，只做适度提炼和页级主旨整理。
- 用户确认大纲后，才能开始生成 `sources/` 文件和最终 HTML。
- 不要手写最终的 `index.html`。
...
生成 PPT 前，智能体应该先和用户对齐内容与风格，而不是直接开始写 HTML。
...
如果用户只给了一个标题或一句话需求，智能体应该先补全一个合理的大纲，包括：
