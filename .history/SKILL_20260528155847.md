...
17. Do a 13-inch laptop layout sanity check mentally or visually: the deck must remain readable at `1366x768`, and should still preserve top/bottom breathing room at `1280x720`. Do not optimize for phone or extremely narrow windows if that would weaken keynote composition.
18. If the final answer or environment requires a single downloadable HTML artifact, read the merged `<deck-folder>/index.html` and export/save exactly that merged content. Never export a hand-written substitute.
...
- Read the already merged `<deck-folder>/index.html`.注意，不要将单页面传入导入工具方法`save_output_file`，只把最后合并的文件传入导出保存的方法中。
