...
平台已禁用 Bash 工具,**不能**通过命令行执行 `python scripts/generate_docx.py ...`。

请改用专门的工具 `run_skill_script` 调用脚本,直接传函数参数即可:

```
工具: run_skill_script
参数:
  skill: "gongwnexiezuo"
  script: "scripts/generate_docx.py"
  output_filename: "关于xx的通知.docx"   ← 用户最终看到的文件名
  kwargs:
    title: "关于xx的通知"
    issuer: "xxx办公室"
    date: "2025-05-04"
    recipient: "各部门"
    content: "<正文 Markdown,带 \\n 换行,小标题用 一、 二、 编号>"
    attachments: ""    # 可选,多个用 | 分隔
```

调用成功后,工具会返回 `{ok: true, file: {download_url, name, size}}`,前端会自动显示一张文件卡片(带下载和预览按钮),无需再把内容粘贴给用户。

**绝对不要**做这些:
- 说"我无法执行 Python 脚本" — 你可以,通过 `run_skill_script`
- 说"我刚才生成的是占位符" — 调一次工具就有真实文件
- 把整段 Markdown 公文打字给用户 — 一律走 `run_skill_script`
